...
int main()
{
    ...
    std::cin >> popInc;
    while (popInc < 0) {
        std::cout << std::setw(50) << std::left << "Please enter a positive increase percentage:" << std::right << '%';
        std::cin.clear();
        std::cin.ignore(256, '\n');
        std::cin >> popInc;
    }
    ...
    while (days < 1 || (days - floor(days)) != 0) {
        if (days < 1) {
            std::cout << std::setw(51) << std::left << "Please enter a number of days greater than 1: ";
            std::cin.clear();
            std::cin.ignore(256, '\n');
            std::cin >> days;
        }
        if ((days - floor(days)) != 0) {
            std::cout << std::setw(51) << std::left << "Please enter a whole number of days: ";
            std::cin.clear();
            std::cin.ignore(256, '\n');
            std::cin >> days;
        }
    ...
    for (int i = 1; i <= days; i++) {
        ...
        std::cout << std::fixed << std::setprecision(2) << "\nThe population after " << i << std::setw(17) << std::left << " days is: " << pop;
    }
